...
import { createPartnerDocSignedUrl } from './partnerDocs';
...
export const PartnerAdmissions: React.FC = () => {
  ...
  const handleApprove = async (inquiry: PartnerInquiry) => {
    ...
    const confirmed = await confirmAsync(`${inquiry.company_name} 업체의 입점을 승인하시겠습니까?`, '입점 승인 확인');
    if (!confirmed) return;
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      await approvePartner({
        ...
        action: 'reject',
        rejectionReason: rejectReason.trim() || '운영팀에 문의 부탁드립니다.',
      });
  ...
  const handleOpenBusinessLicense = async (inquiry: PartnerInquiry) => {
    ...
    try {
      const signedUrl = await createPartnerDocSignedUrl(client, storedValue);
  ...
  const filtered = facilities.filter((facility) =>
    facility.company_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (facility.contact_person || '').toLowerCase().includes(searchTerm.toLowerCase()),
  );
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((facility) => (
            <div
              ...
              className="bg-white p-6 rounded-xl border shadow-sm flex flex-col md:flex-row gap-6 items-start md:items-center justify-between hover:border-blue-200 transition-all"
            >
              <div className="flex-1 space-y-2 w-full">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="text-lg font-bold text-gray-900">{facility.company_name}</h3>
                  <div className="flex items-center gap-1">
                    <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded border border-gray-200 font-medium whitespace-nowrap">
                      {facility.business_type === 'funeral_home'
                        ? '장례식장'
                        : facility.business_type === 'memorial_park'
                          ? '묘지/봉안당'
                          : facility.business_type === 'sangjo'
                            ? '상조회사'
                            : facility.business_type === 'pet_funeral'
                              ? '반려동물 장례'
                ...
                <div className="text-sm text-gray-500 space-y-1">
                  <div className="flex flex-wrap gap-x-4 gap-y-1">
                    <p className="flex items-center gap-1.5">
                      <User size={14} className="text-gray-400 shrink-0" /> {facility.contact_person} ({facility.manager_mobile})
                    </p>
                    <p className="flex items-center gap-1.5">
                      <Phone size={14} className="text-gray-400 shrink-0" /> {facility.contact_number}
                    </p>
                    <p className="flex items-center gap-1.5">
                      <Building2 size={12} className="text-gray-400 shrink-0" /> {facility.address}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-x-4 text-xs">
                    <p className="text-gray-400">Email: {facility.company_email}</p>
                    <p className="text-gray-400">요청일: {new Date(facility.created_at).toLocaleDateString()}</p>
                  </div>
                  {facility.business_license_url && (
                    <div className="mt-2">
                      <button
                        type="button"
                        onClick={() => handleOpenBusinessLicense(facility)}
                        disabled={openingLicenseId === facility.id}
                        className="text-blue-500 text-xs hover:underline flex items-center gap-1 disabled:opacity-50"
                      >
                        <FileText size={12} />
                        {openingLicenseId === facility.id ? '문서를 여는 중..' : '사업자등록증 보기'}
                      </button>
                    </div>
                  )}
                  {facility.message && (
              ...
              <div className="flex flex-wrap gap-2 w-full md:w-auto mt-4 md:mt-0 justify-end">
                ...
                <button
                  onClick={() => {
                    setRejectTarget({ id: facility.id, name: facility.company_name });
                    setRejectReason('');
                  }}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            <textarea
              ...
              placeholder="거절 사유를 입력해 주세요. 미입력 시 '운영팀에 문의 부탁드립니다.'가 사용됩니다."
              rows={3}
