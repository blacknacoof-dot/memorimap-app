...
const PURPOSES = ['안치 상담', '장례 상담', '시설 견학', '기타'];
const PURPOSES_MEMORIAL = ['안치 상담', '봉안 상담', '자연장 상담', '기일 추모'];
...
export const StepInfo: React.FC<Props> = ({
  ...
  <div className="space-y-4">
    <div>
      ...
      <input {...register('visitor_name')} data-testid='reservation-visitor-name' className="w-full p-3 border rounded-lg focus:ring-2 outline-none" placeholder="성함" />
    ...
    <div>
      ...
      <textarea {...register('request_note')} className="w-full p-3 border rounded-lg h-20 resize-none outline-none" placeholder="요청 사항" />
    </div>
  </div>
);
