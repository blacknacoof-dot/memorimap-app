...
const FuneralSearchForm: React.FC<FormProps> = ({
    ...
    // Recommendations
    const [recommendedFacilities, setRecommendedFacilities] = useState<Array<{ id: string; name: string; address?: string; phone?: string; image_url?: string; imageUrl?: string; images?: string[] }>>([]);
    ...
    const handleConsultSubmit = async (data: Record<string, unknown>) => {
        ...
        const scaleVal = (data.scale as string) || '미선택';
        const religionVal = (data.religion as string) || '미선택';
                                        ...
                                        <div className="flex gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex flex-col items-start gap-1">
                                                    <button onClick={() => onSwitchToFacility?.({ id: f.id, name: f.name, address: f.address, phone: f.phone })} className="w-full text-[15px] font-bold text-indigo-700 hover:underline text-left leading-tight whitespace-normal break-keep">{f.name}</button>
                                                    <span className="bg-indigo-50 text-indigo-600 text-[10px] px-2 py-0.5 rounded-full font-bold shrink-0">추천 {idx + 1}</span>
                                                </div>
