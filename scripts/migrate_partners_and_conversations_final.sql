...
-- =================================================================
-- 1. DROP EXISTING POLICIES (Blocking Migration)
-- =================================================================
-- Partners
...
-- Drop FK constraint that blocks type change
...
-- 2.2 Delete Invalid/Test/Legacy Data (partner_conversations)
-- Delete if user_id is not UUID OR partner_id is not UUID (orphaned from test partners)
...
-- 2.3 Delete Invalid/Test Data (partners)
...
ALTER COLUMN id TYPE uuid USING id::uuid; -- Assuming id is already uuid or needs migration? Usually id is uuid default gen_random_uuid() but check check_partners_id_type didn't check conv id. Assuming safe or explicit cast.
ALTER TABLE partner_conversations
...
-- Partner Conversations: Standard Access
