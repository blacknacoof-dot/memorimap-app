...
function parseText(text: string) {
    ...
    for (const block of blocks) {
        ...
        const isLine0Type = ['봉안시설', '자연장', '동물장묘', '장례식장'].some(k => lines[0].includes(k));
        ...
        // Clean address
        if (address && address.startsWith('★')) address = lines[lines.length - 3];
        // If address is undefined, try finding "시 "
        if (!address) {
            address = lines.find(l => l.includes('시 ') || l.includes('군 ') || l.includes('구 ')) || '';
        }

        let dbType = 'charnel'; // Default
        if (rawType.includes('자연') || name.includes('수목') || name.includes('잔디') || rawType.includes('자연장')) dbType = 'natural';
...
async function main() {
    console.log("🚀 대규모 시설 추가 및 검증 (Retry) 시작...");
