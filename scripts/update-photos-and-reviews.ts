...
// 리뷰 생성
const generateReview = (facilityId: number): any => {
...
async function updatePhotosAndReviews() {
    console.log("🎨 신규 시설 사진 및 리뷰 업데이트 시작...\n");

    // 1. 네이버 API로 추가된 시설 조회
    ...
    for (let i = 0; i < facilities.length; i++) {
        ...
        const reviews = [];
        for (let j = 0; j < 5; j++) {
            reviews.push(generateReview(facility.id));
        }
