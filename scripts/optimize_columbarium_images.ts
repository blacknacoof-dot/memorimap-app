...
const SOURCE_DIR = path.resolve(__dirname, '../data/봉안시설 대표이미지');
const OUTPUT_DIR = path.resolve(__dirname, '../data/봉안시설 대표이미지_최적화');
...
async function optimizeAllImages() {
    ...
    console.log('🏛️  봉안시설 이미지 최적화 (Columbarium Image Optimization)');
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const [index, file] of files.entries()) {
            ...
            const outputName = `columbarium_${index + 1}.jpg`;
        ...
        console.log();
        console.log('🚀 다음 단계:');
        console.log('   npx tsx scripts/upload_columbarium_images.ts');
        console.log('='.repeat(60));
