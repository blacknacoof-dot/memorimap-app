...
const SANGJO_114_COMPANY_KEY = "\uc0c1\uc870114";
...
export function getSangjoRepresentativeImage(candidates: RepresentativeImageCandidates): string {
  const companyImageOverride = getCompanyImageOverride(candidates.companyName);

  if (normalizeCompanyName(candidates.companyName) === SANGJO_114_COMPANY_KEY && companyImageOverride) {
    return companyImageOverride;
  }
