-- ========================================================================
-- Facility Reviews RLS 정책 최종 통합 해결 스크립트 (V7 - Profiles Aware)
-- ========================================================================
-- 문제 원인: Clerk 사용자는 Supabase auth.jwt()의 user_metadata에 role이 없습니다.
-- 대신 public.profiles 테이블에 role 정보가 있으므로, 이를 참조하도록 정책을 수정합니다.
-- 또한 'super_admin' 권한도 명시적으로 포함합니다.
-- ========================================================================
...
-- 2. SELECT (조회): 활성 리뷰는 누구나, 전체는 관리자(admin/super_admin)
...
DO $$ 
BEGIN
    RAISE NOTICE '✅ V7 Profiles Aware RLS 정책 적용 완료';
END $$;
