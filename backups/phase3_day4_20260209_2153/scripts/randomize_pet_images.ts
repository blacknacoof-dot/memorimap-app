...
const SOURCE_DIR = path.resolve(rootDir, 'data/반려동물장례');
const BUCKET = 'facility-images';
const STORAGE_PREFIX = 'pet_random';
...
async function run() {
    console.log('🐶 반려동물장례 이미지 랜덤 배분 시작...');
    ...
    // 타겟 필터: 이미지 없음 / 빈 배열 / 기본 이미지(defaults/pet) / placeholder
    // 사용자 요청: "같은것 쓰거나 없는곳"
    const targets = facilities.filter(f => {
        if (!f.images || f.images.length === 0) return true;
        // 기본 이미지 체크
        return f.images.some((url: string) =>
            url.includes('defaults/pet') ||
            url.includes('defaults/charnel') ||  // 혹시 잘못 들어간 것
            url.includes('placeholder')
        );
        // 자체 이미지(Google 등)가 있는 곳은 건드리지 않음
    });
