-- =====================================================
-- Memorimap System Normalization Script (v1.1)
-- Includes: Super Admins, Audit Logs, Facilities, RLS Fixes
-- Update: Added approve_partner_transaction RPC
-- =====================================================

-- 0. Ensure Foundation Tables Exist
...
-- 1. Security Definer Function (Fixing RLS Recursion)
...
CREATE TABLE IF NOT EXISTS public.facilities (
    ...
    verified BOOLEAN DEFAULT false, -- Matches schema
...
-- 5. RLS Policies (Applying is_super_admin)
...
-- 6. RPC: Atomic Transaction for Partner Approval
...
BEGIN
    ...
    VALUES (
        ...
        CASE 
            ...
            ELSE 'sangjo_biz' -- Default type if not funeral_home
        END,
        COALESCE(v_inquiry.address, ''),
        COALESCE(v_inquiry.company_phone, v_inquiry.contact_number, ''),
        true, -- verified
        'active',
        '{}'::jsonb,
        now()
    ...
    -- Checking 'sangjo' OR 'sangjo_hq' to cover both cases
...
-- 7. Helper for logs (used by Edge Function if needed)
