-- =====================================================
-- MASTER FIX: ID Type & RPC Search Function (v2 - Corrected Column Names)
-- =====================================================

BEGIN;

-- 1. [Critical] Change ID column types from UUID to TEXT for Clerk compatibility
-- Note: 'facilities' now uses 'user_id' instead of 'manager_id' after v4 normalization.
ALTER TABLE public.super_admins ALTER COLUMN id TYPE text;
ALTER TABLE public.facilities ALTER COLUMN user_id TYPE text; -- Corrected from manager_id
...
DROP FUNCTION IF EXISTS search_facilities(float, float, int, text, text);
